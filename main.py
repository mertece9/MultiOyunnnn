...
def anlam_getir(kelime):
    js = json.loads(requests.get("https://sozluk.gov.tr/gts?ara="+kelime, headers = headers).content.decode())[0]["anlamlarListe"]
...
async def start_private(message): #, **kwargs
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            callback_button = types.InlineKeyboardButton(text="🇹🇷 ʙᴇɴɪ ɢʀᴜʙᴀ ᴇᴋʟᴇ 🇹🇷", url="https://t.me/LuciBeyVipBot?startgroup=a")
            callback_button2 = types.InlineKeyboardButton(text="⚙️ ʀᴇsᴍɪ ᴋᴀɴᴀʟ ⚙️", url="https://t.me/LcBy35")
...
async def kelime_turet_baslat(message, **kwargs):
    ...
    if konumlar != False:
        ...
        bot.send_message(chat_id, f'❌ Sayın <a href="tg://user?id={user_id}">{first_name}</a>, şu anda aktif oyun var.')
        return
    ...
    while msg_durum:
        try:
            ...
            if "Too Many" in str(e):
                ...
                bot.send_message(chat_id, "❌ Hata oluştu. Lütfen tekrar deneyin.")
                bot.send_message(kurucu_id, str(e))
                bot.send_message(kurucu_id, get_traceback(e))
                break
    #yap()
...
Herhangi bir kişi alıntılanmamış!
        ...
        bot.send_message(chat_id, f"Lütfen komutu şöyle giriniz: /oban {ayrik[2]}", reply_markup=keyboard, reply_to_message_id=message.id)
...
Bulduğu cevaplar: \t{f(f"privates.{user_id}.bilme-sayısı")}
            ...
            if tip == "kureselskor":
                ...
                for n,i in enumerate(skorlar_list):
                    ...
                    if firstname != "":
                        txt += f"<b>{n+1}</b>. {firstname} - <code>{i[1]:.0f}</code> puan" #:.2f
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1001948240595, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
    ...
    if sorgu == "kelimeturet_harf":
        ...
        if sorgu.startswith("kelimeoyunu_"):   
            ...
            if len(ayir) == 2:
                ...
                keyboard = types.InlineKeyboardMarkup()
                callback_button1 = types.InlineKeyboardButton(text="🧠 Kolay (x1 puan)", callback_data=kolay_callback)
                callback_button2 = types.InlineKeyboardButton(text="💣 Zor (x4 puan)", callback_data=zor_callback)
                keyboard.add(callback_button1, callback_button2)
        ...
        if sorgu == "ipucu_kelime":
            ...
            try:
                ...
                anlamlar = "\n".join([f"✍️ Tanım {say+1}: {i}" for say, i in enumerate(random.sample(anlam,min(2,len(anlam))))])
                #tdk_getir = tdk.gts.search(rastgele_kelime)[0]
                #for say in range(2):
                #    try:
                #        anlamlar = anlamlar + f"✍️ <b>Tanım {say+1}: </b>" + tdk_getir.meanings[say].meaning + "\n"     
...
<a href="tg://user?id={user_id}"><b>{first_name}</b></a> doğru bildi''', reply_markup=keyboard)
            ...
            #keyboard.add(callback_button)
                ...
                if bilme_sayisi % 100 == 0:
                    ...
                    await bot.send_message(chat_id,f'Değerli {first_name} ilk kez bildi! 🌟') # ve sorusunu anlatıyor
                ...
                if random.randint(0,2) == 0:
                    ...
                    keyboard = types.InlineKeyboardMarkup()
                    callback_button = types.InlineKeyboardButton(text="Sunucu olmak istiyorum.", callback_data=f'istiyorum_sessiz_sinema_{mod}')
                    keyboard.add(callback_button)
                ...
                if oyun_tipi == "sessiz_sinema":
                    ...
                    if soru_suresi < int(time.time()) - id/zaman_hassasiyeti:
                        ...
                        if str(id) == str(f(f"groups.{konum}.oyun")):
                            ...
                            keyboard = types.InlineKeyboardMarkup()
                            callback_button = types.InlineKeyboardButton(text="Sunucu olmak istiyorum! 🙋🏻🙋🏻‍♀️", callback_data="istiyorum_"+oyun_modu)
                            keyboard.add(callback_button)
...
#
                        await bot.send_message(konum, random.choice([
                            ...
                            f"Sana çiçek vermek istiyorum, {user} → " + random.choice(["🌼", "🌷","🌻", "🌼","🌺","🌸", "🌹"]) +". Peki şimdi anlatır mısın?",
                        ...
                        keyboard = types.InlineKeyboardMarkup()
                        callback_button = types.InlineKeyboardButton(text="Sunucu olmak istiyorum! 🙋🏻🙋🏻‍♀️", callback_data="istiyorum")
                        keyboard.add(callback_button)
#
                        await bot.send_message(konum, random.choice([
    ...
    while 1:
        try:
            ...
            try:
                ...
                bot.send_message(skor, "🎖 Bu haftanın en iyi skoruna sahip oyuncusu seçildiniz, Tebrikler!")
            except Exception as e:
                bot.send_message(kurucu_id, str(e))
            ...
            try:
                ...
                skorlar = ww
                skor = sorted(skorlar.items(), key=lambda item: item[1])[::-1][0]
                bot.send_message(skor, "🎖 Bu haftanın en iyi skoruna sahip grubu seçildiniz!")
            except Exception as e:
                bot.send_message(kurucu_id, str(e))
            ...
            for i in groups:
                ...
                try:
                    ...
                    if "chat not found" in str(e):
                        bot.send_message(kurucu_id,f"{i} grubunu sildim çünkü chat not found diyor")
                        #f(f"groups.{i}","$del")
                        return
